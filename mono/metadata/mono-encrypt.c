...
static uint8_t g_Flags[16] = { 0 };
static uint8_t g_key[32] = { 0 };
static uint8_t g_iv[32] = { 0 };

// generate key: "BODONG++??D*YY??"
static void CalcFlags()
{
	...
	if (!HasGen)
	{
		...
		for (i = 0; i < sizeof(g_Flags) / sizeof(g_Flags[0]); ++i)
		{
			g_Flags[i] = data[i] ^ (0x27 - i);
		}
	}

#if DEBUG
	g_assert(strncmp(g_Flags, "BODONG++??D*YY??", sizeof(data)) == 0);
#endif
}

// generate key: "A1CD#&($zyDk&&&@"
static void CalcKey()
{
	int i;
	static int HasGen = 0;
	...
	if (!HasGen)
	{
		...
		{
			g_key[i] = data[i] ^ (0x27 - i);
		}
	}

#if DEBUG
	g_assert(strnicmp(g_key, "A1CD#&($zyDk&&&@", sizeof(data)) == 0);
#endif
}

// generate key: "&kdj*^%*dktdM053"
static void CalcIV()
{
	int i;
	static int HasGen = 0;
	...
	if (!HasGen)
	{
		...
		for (i = 0; i < sizeof(data) / sizeof(data[0]); ++i)
		{
			g_iv[i] = data[i] ^ (0x27 - i);
		}
	}

#if DEBUG
	g_assert(strnicmp(g_iv, "&kdj*^%*dktdM053", sizeof(data)) == 0);
#endif
}
...
int __stdcall mono_is_buffer_encrypted(char* data, size_t size)
{
	...
	if (size < sizeof(g_Flags))
	{
		return 0;
	}

	for (i = 0; i < sizeof(g_Flags) / sizeof(uint8_t); ++i)
	{
		if (data[i] != g_Flags[i])
		{
			return 0;
...
{
	uint8_t ReadFlag[sizeof(g_Flags) / sizeof(uint8_t)];
	FILE* file;
	long fileLength;
	int i;
	...
	if (fileLength <= sizeof(g_Flags))
	{
		fclose(file);
		return 0;
	}

	fread(ReadFlag, 1, sizeof(ReadFlag), file);

	fclose(file);

	return mono_is_buffer_encrypted(ReadFlag, sizeof(ReadFlag) / sizeof(ReadFlag[0]));
}

int __stdcall mono_assembly_get_data_from_data(char* data, size_t size, void** decrypted_data, uint64_t* decrypted_data_len)
{
	int i;
	struct AES_ctx ctx;
	char* Data;
	...
	for (i = 0; i < sizeof(g_Flags) / sizeof(uint8_t); ++i)
	{
		...
		{
			*decrypted_data = data;
			*decrypted_data_len = size;

			return 1;
		}
	}

	dataLength = size - sizeof(g_Flags);
	Data = malloc(dataLength);
	memcpy(Data, data + sizeof(g_Flags), dataLength);

	AES_init_ctx_iv(&ctx, g_key, g_iv);
	AES_CBC_decrypt_buffer(&ctx, Data, dataLength);

	realLength = pkcs7_padding_data_length(Data, dataLength, 16);
...
__declspec(dllexport) int __stdcall mono_assembly_get_data(char* filePath, void** decrypted_data, uint64_t* decrypted_data_len)
{
	...
	*decrypted_data_len = 0;
	...
	for (i = 0; i < sizeof(ReadFlag); ++i)
	{
		if (ReadFlag[i] != g_Flags[i])
		{
			fseek(file, 0, SEEK_SET);
			...
			*decrypted_data = Data;
			*decrypted_data_len = fileLength;

			return 1;
		}
	}

	dataLength = fileLength - sizeof(g_Flags);
	Data = malloc(dataLength);
...
#else
int __stdcall mono_encrypt_assembly_file(char* filePath, char* outFile)
#endif
{
	FILE* file;
	long fileLength;
	...
	padByte = pkcs7_padding_pad_buffer(Data, fileLength, dataLength, 16);

	AES_init_ctx_iv(&ctx, g_key, g_iv);
	AES_CBC_encrypt_buffer(&ctx, Data, dataLength);

	file = fopen(outFile, "wb");
	assert(file);

	fwrite(g_Flags, 1, sizeof(g_Flags), file);
...
#else
int __stdcall mono_decrypt_assembly_file(char* filePath, char* outFile)
#endif
{	
	...
	if (mono_assembly_get_status(filePath) <= 0)
	{
		return 0;
	}

	if (mono_assembly_get_data(filePath, &decrypted_data, &decrypted_data_len) > 0 && decrypted_data != NULL)
	{
		file = fopen(outFile, "wb");
		fwrite(decrypted_data, 1, decrypted_data_len, file);
		fclose(file);
...
void mono_des_assembly_file(char* filePath)
{
	void* decrypted_data = NULL;
	uint64_t decrypted_data_len = 0;
	FILE* file;

	if (mono_assembly_get_status(filePath))
	{
		if (mono_assembly_get_data(filePath, &decrypted_data, &decrypted_data_len) > 0 && decrypted_data != NULL)
		{
	...
	{
		mono_encrypt_assembly_file(filePath, filePath);
	}
}
